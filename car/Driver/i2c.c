...
#define I2C_RECOVERY_PULSES        (9U) // SDA 被从机拉低时，最多补发 9 个 SCL 脉冲释放总线
#define I2C_RECOVERY_DELAY_CYCLES  (320U) // 软件恢复总线时相邻电平变化之间的短延时

volatile uint8_t g_i2c_last_stage = 0; // 最近一次异常发生在哪个检查阶段，便于断点观察
volatile uint8_t g_i2c_last_addr = 0; // 最近一次访问的 I2C 从机地址
volatile uint8_t g_i2c_last_reg = 0; // 最近一次访问的寄存器地址
volatile uint8_t g_i2c_last_len = 0; // 最近一次访问的数据长度
volatile uint32_t g_i2c_last_status = 0; // 异常发生时的 I2C 控制器状态
volatile uint32_t g_i2c_last_rawint = 0; // 异常发生时的 I2C 原始中断状态

static uint8_t s_i2c_current_addr = 0; // 当前正在访问的从机地址
static uint8_t s_i2c_current_reg = 0; // 当前正在访问的寄存器地址
static uint8_t s_i2c_current_len = 0; // 当前正在访问的数据长度

static void i2c_set_current(uint8_t addr, uint8_t reg, uint8_t len)
{
    s_i2c_current_addr = addr; // 保存当前事务，异常时能知道失败的是哪次访问
...
{
    g_i2c_last_stage = stage; // 保存现场，不在底层直接打印，避免阻塞控制任务
...
{
    DL_GPIO_disableOutput(GPIO_I2C_1_SDA_PORT, GPIO_I2C_1_SDA_PIN); // 切成输入，相当于开漏释放 SDA
...
{
    DL_GPIO_disableOutput(GPIO_I2C_1_SCL_PORT, GPIO_I2C_1_SCL_PIN); // 切成输入，相当于开漏释放 SCL
...
static void i2c_restore_pins(void)
{
    ...
    DL_GPIO_initPeripheralInputFunctionFeatures(GPIO_I2C_1_IOMUX_SDA, // 软件恢复结束后交还给 I2C1 外设
...
{
    bool recovered; // 记录 SDA、SCL 最终是否都回到高电平

    DL_I2C_disableController(I2C_1_INST); // 临时关闭硬件 I2C，改由 GPIO 手动释放总线
    i2c_reset_transfer();
    ...
    for (uint8_t i = 0; (i < I2C_RECOVERY_PULSES) && !i2c_sda_high(); i++) { // 给卡住的从机补时钟
        i2c_drive_scl_low();
        i2c_short_delay();
        i2c_release_scl();
        i2c_short_delay();
    ...
    i2c_drive_sda_low(); // 手动产生 STOP：SCL 为高时释放 SDA
    i2c_short_delay();
    i2c_release_scl();
    i2c_short_delay();
    i2c_release_sda();
    ...
    i2c_restore_pins();
    SYSCFG_DL_I2C_1_init(); // 恢复 I2C1 寄存器配置
    i2c_reset_transfer();
    i2c_short_delay();
...
{
    uint8_t i2c_buf[2] = {reg_address, data}; // 先发寄存器地址，再发要写入的值
...
{
    uint8_t i2c_buf[32]; // MPU6050 初始化只需要较短写入，固定缓存避免动态分配
...
{
    DL_I2C_CONTROLLER_ACK ack; // 单字节读取最后一字节不回 ACK，多字节读取中间字节回 ACK
    ...
    DL_I2C_disableInterrupt(I2C_1_INST, DL_I2C_INTERRUPT_CONTROLLER_TXFIFO_TRIGGER);
    DL_I2C_enableControllerReadOnTXEmpty(I2C_1_INST); // 寄存器地址发完后自动切换到读取阶段
