...
2. 上电默认空闲，按键切到循迹模式后，`Line_Follow` 任务每 5 ms 解析循迹数据并计算车身相对黑线的偏差。
...
7. 按键通过 GPIOB 中断唤醒 `KEY` 任务，切换 `button_task` 模式变量。
...
- 上电默认 `BUTTON_TASK_IDLE`，未按键进入 `BUTTON_TASK_LINE_FOLLOW` 前保持停车，不解析循迹帧。
- 收到新帧则更新循迹状态。
- 500 ms 内没有新循迹帧，或当前没有可靠偏差，则左右轮目标速度置 0。
- 有效偏差下使用 `turn = error * LINE_FOLLOW_KP + delta_error * LINE_FOLLOW_KD` 计算差速。
- 差速经过 `LINE_FOLLOW_TURN_RPM` 限幅和平滑后，通过 `control_set_target_rpm()` 设置左右轮目标转速：

```text
left_target  = LINE_FOLLOW_BASE_RPM - turn
right_target = LINE_FOLLOW_BASE_RPM + turn
```
...
| 参数 | 值 | 说明 |
| --- | --- | --- |
| `LINE_FOLLOW_BASE_RPM` | 100.0 | 基础前进速度 |
| `LINE_FOLLOW_KP` | 10.0 | 循迹比例系数 |
| `LINE_FOLLOW_KD` | 2.0 | 循迹微分系数 |
| `LINE_FOLLOW_TURN_RPM` | 100.0 | 转向修正限幅 |
| `LINE_FOLLOW_PERIOD_MS` | 5 ms | 循迹任务周期 |
| `LINE_FOLLOW_TIMEOUT_MS` | 500 ms | 循迹帧超时停车时间 |

### 4.2 双轮速度闭环

`control.c` 在 `TIMER_0_INST_IRQHandler()` 中完成 10 ms 闭环：

1. 调用 `encoder_get_total()` 获取左右轮累计计数，并在 `control.c` 内和上一次快照做差得到本周期计数。
2. 按实测每圈计数换算 RPM。
...
当前主任务链路中没有周期调用串口调参入口。左右轮 PID 由 `control.c` 私有持有；如需启用串口调参，在合适任务中调用 `control_read_pid_uart()`，并注意它与循迹帧共用 UART 接收缓存。
...
当前上电默认 `BUTTON_TASK_IDLE`，第一次按键切到 `BUTTON_TASK_LINE_FOLLOW` 后才允许循迹任务解析循迹帧并下发左右轮目标速度。继续按键会沿用模式轮转，离开循迹模式时 `Line_Follow` 会保持停车。
...
当前主流程已经接入：电机 PWM、编码器测速、10 ms 速度 PID、按键启动循迹、UART 循迹解析、5 ms 循迹任务、OLED 遥测显示、按键中断消抖。

当前已实现但尚未接入主控制流程：MPU6050/DMP 姿态读取、串口 PID 调参周期调用。
